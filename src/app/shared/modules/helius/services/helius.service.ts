...
export class HeliusService implements IRpc {
	...
	readonly accounts = new Map<string, CommitmentTypeEnum>();

	readonly messageSubject = new Subject();
	...
	init() {
		...
		this._loggerService.log("Helius запущен");
		...
		this._ws.on("message", async (messageBuffer: WebSocket.Data) => {
			...
			if (message.error || message.params?.error) {
				this._loggerService.error(message.error || message.params?.error);
		...
		this._ws.on("error", (error) => {
			this._loggerService.error(`Helius ошибка: ${error.message}`);

			this._eventsService.emit(EventsEnum.HELIUS_ERROR, error.message, true);
		});
		this._ws.on("close", (errorCode) => {
			this._loggerService.error(`Helius соединение закрыто: ${errorCode}`);
